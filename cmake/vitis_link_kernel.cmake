...
#
# :param OUTPUT: Output name of the kernel to be linked (placed & routed) :type
# target: string
#
# :param CONFIG: CMAKE_SOURCE_DIR relative path to configuration file for the
# kernel e.g. src/zcu102.cfg. :type target: string
#
# :param DTSI (optional): CMAKE_SOURCE_DIR relative path to dtsi file for the
# resulting linked kernel e.g. src/vadd_faster.dtsi. :type target: string
#
# :param KERNELS: two or more CMAKE_BINARY_DIR relative paths for the kernels to
# link. :type target: string
#
# Example:
#
# vitis_link_kernel( OUTPUT image_proc KERNELS resize_accel rectify_accel CONFIG
# src/kv260.cfg )
#
macro(vitis_link_kernel)
  ...
  cmake_parse_arguments(VITIS_LINK "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  if(DEFINED CMAKE_SYSROOT)
    if(${NOKERNELS})
      ...
      foreach(include_item ${VITIS_LINK_KERNELS})
        list(APPEND KERNELS_INCLUDE "${CMAKE_BINARY_DIR}/${include_item}.xo")
      endforeach()

      # place and route
      set(ENV{PLATFORM_REPO_PATHS} ${PLATFORM_REPO_PATHS})
      set(CMD
          "${VPP_PATH} -l -t hw --config ${CMAKE_SOURCE_DIR}/${VITIS_LINK_CONFIG} ${KERNELS_INCLUDE} -o ${CMAKE_BINARY_DIR}/${VITIS_LINK_OUTPUT}.xclbin "
      )
      # message(WARNING ${CMD}) run(${CMD})
      execute_process(
        COMMAND
          ${VPP_PATH} -l -t hw --config ${CMAKE_SOURCE_DIR}/${VITIS_LINK_CONFIG}
          -o ${CMAKE_BINARY_DIR}/${VITIS_LINK_OUTPUT}.xclbin ${KERNELS_INCLUDE}
      ...
      # generate Vitis Accelerated Application (AA) files, if linking was
      # successful
      if(EXISTS "${CMAKE_BINARY_DIR}/${VITIS_LINK_OUTPUT}.xclbin")
        ...
        install(
          FILES "${CMAKE_BINARY_DIR}/${VITIS_LINK_OUTPUT}.xclbin"
                "${CMAKE_BINARY_DIR}/${VITIS_LINK_OUTPUT}.dtbo"
