# ____  ____ /   /\/   / /___/  \  /   Copyright (c) 2021, Xilinx®. \   \   \/
# Author: Víctor Mayoral Vilches <victorma@xilinx.com> \   \ /   / /___/   /\ \
# \  /  \ \___\/\___\
#
...
#
# .. note:: call sub-macros or sub-functions to build, link and/or package, as
# appropriate.
#
# .. note:: kernels will be generated with their corresponding ".xclbin" format.
# A signature of the build type will be appended at the end except for the
# "hardware" build type which includes no signature. E.g. "vadd.xclbin.hw_emu"
# or "vadd.xclbin".
#
# :param NAME: Name of the kernel to compile and link :type target: string
#
# :param FILE: CMAKE_SOURCE_DIR relative path to source code of the kernel :type
# target: string
#
# :param CONFIG: CMAKE_SOURCE_DIR relative path to configuration file for the
# kernel e.g. src/zcu102.cfg. :type target: string
#
# :param DTSI (optional): Device Tree Source Include files, define additions to
# the device tree so that the corresponding acceleration kernel can operate
# appropriately. :type target: string
#
# :param SHELLJSON (optional): describes the base shell configuration
# information of the acceleration kernel. See
# https://github.com/Xilinx/dfx-mgr/tree/xlnx_rel_v2021.2#shelljson :type
# target: string
#
# :param CLOCK: Frequency in Hz at which the kernel should be compiled by Vitis
# HLS. See
# https://www.xilinx.com/html_docs/xilinx2020_2/vitis_doc/vitiscommandcompiler.html#mcj1568640526180__section_bh5_dg4_bjb
# for more details. :type target: string
#
# :param TYPE: type of kernel to generate, could be "sw_emu", "hw_emu" or
# "hardware" (or several of them) :type target: string
#
...
# are relative to CMAKE_SOURCE_DIR. :type target: string
#
# :param LINK: Invoke the v++ linker to generate the corresponding artifacts.
# :type target: bool
#
# :param PACKAGE: Invoke the -p (package) flag of v++ to generate the
# corresponding artifacts for emulation. :type target: bool
#
# Example:
#
# vitis_acceleration_kernel( NAME vadd_faster FILE src/vadd.cpp CONFIG
# src/kv260.cfg DTSI src/vadd_faster.dtsi CLOCK 100000000:vadd_faster INCLUDE
# include TYPE hw LINK PACKAGE )
#
    ...
    foreach(type ${VITIS_KERNEL_TYPE})
      ...
      elseif(${type} STREQUAL "hw")
        ...
        message(
          FATAL_ERROR
            "'"
            ${type}
            "' is not a recognized build target \
      ...
      if(${VITIS_KERNEL_LINK} AND ${VITIS_KERNEL_PACKAGE})
        vitis_acceleration_kernel_aux(
          ...
          ${VITIS_KERNEL_DTSI}
          SHELLJSON
          ${VITIS_KERNEL_SHELLJSON}
          INCLUDE
          ${VITIS_KERNEL_INCLUDE}
          LINK
          PACKAGE)
...
# the kernel :type target: string
#
# :param VITIS_KERNEL_AUX_CONFIG: CMAKE_SOURCE_DIR relative path to
# configuration file for the kernel e.g. src/zcu102.cfg. :type target: string
#
# :param VITIS_KERNEL_AUX_DTSI: CMAKE_SOURCE_DIR relative path to dtsi file for
# the kernel e.g. src/vadd_faster.dtsi. :type target: string
#
# :param VITIS_KERNEL_AUX_SHELLJSON: CMAKE_SOURCE_DIR relative path to
# shell.json file for the acceleration kernel e.g. src/shell.json. :type target:
# string
#
# :param VITIS_KERNEL_AUX_CLOCK: Frequency in Hz at which the kernel should be
# compiled by Vitis HLS. See
# https://www.xilinx.com/html_docs/xilinx2020_2/vitis_doc/vitiscommandcompiler.html#mcj1568640526180__section_bh5_dg4_bjb
# :type target: string
#
# :param VITIS_KERNEL_AUX_INCLUDE: one or muliple relative path directories
# whereto search for header files. This option is passed to the OpenCL
# preprocessor. Directories are relative to CMAKE_SOURCE_DIR. :type target:
# string
#
# :param VITIS_KERNEL_AUX_LINK: Invoke v++ linker in the the corresponding
# artifacts. :type target: bool
#
# :param VITIS_KERNEL_AUX_PACKAGE: Invoke the -p (package) flag of v++ to
# generate the corresponding artifacts. :type target: bool
#
macro(vitis_acceleration_kernel_aux)
  ...
  set(CMD
      "PLATFORM_REPO_PATHS=${PLATFORM_REPO_PATHS} ${VPP_PATH} -c -t ${VITIS_KERNEL_AUX_TYPE} --config "
      ${VITIS_CONFIGURATION_FILE}
      " -k "
      ${VITIS_KERNEL_AUX_NAME}
      " ")
  set(INCLUDE_DIRS "")
  ...
  if(${NOKERNELS})
    ...
    # Build pass clock flag if available
    if("${VITIS_KERNEL_AUX_CLOCK}" STREQUAL "")
      execute_process(
        COMMAND
          ${VPP_PATH} -c -t ${VITIS_KERNEL_AUX_TYPE} --config
          "${VITIS_CONFIGURATION_FILE}" -k ${VITIS_KERNEL_AUX_NAME}
          ${INCLUDE_DIRS} "${CMAKE_SOURCE_DIR}/${VITIS_KERNEL_AUX_FILE}" -o
          "${CMAKE_BINARY_DIR}/${VITIS_KERNEL_AUX_NAME}.xo"
        RESULT_VARIABLE CMD_ERROR COMMAND_ECHO STDERR
        OUTPUT_VARIABLE VITIS_OUTPUT
        ERROR_VARIABLE VITIS_OUTPUT)
    else()
      execute_process(
        COMMAND
          ${VPP_PATH} -c --hls.clock ${VITIS_KERNEL_AUX_CLOCK} -t
          ${VITIS_KERNEL_AUX_TYPE} --config "${VITIS_CONFIGURATION_FILE}" -k
          ${VITIS_KERNEL_AUX_NAME} ${INCLUDE_DIRS}
          "${CMAKE_SOURCE_DIR}/${VITIS_KERNEL_AUX_FILE}" -o
          "${CMAKE_BINARY_DIR}/${VITIS_KERNEL_AUX_NAME}.xo"
        RESULT_VARIABLE CMD_ERROR COMMAND_ECHO STDERR
        OUTPUT_VARIABLE VITIS_OUTPUT
        ERROR_VARIABLE VITIS_OUTPUT)
    ...
    if(CMD_ERROR EQUAL 0)
      message(STATUS "CMD_ERROR: " ${CMD_ERROR} ${VITIS_OUTPUT})
    else()
      message(FATAL_ERROR "CMD_ERROR: " ${CMD_ERROR} ${VITIS_OUTPUT}
      )# returns "0" if all
    endif() # good

    # adjust final binary name if not "hw" (only "hw" build target is without
    # suffix)
    set(BINARY_NAME "${VITIS_KERNEL_AUX_NAME}.xclbin")
    if(NOT ${VITIS_KERNEL_AUX_TYPE} STREQUAL "hw")
      set(BINARY_NAME
          "${VITIS_KERNEL_AUX_NAME}.xclbin.${VITIS_KERNEL_AUX_TYPE}")
    endif()
    ...
    if(${VITIS_KERNEL_AUX_LINK})
      ...
      execute_process(
        COMMAND
          ${VPP_PATH} -l -t ${VITIS_KERNEL_AUX_TYPE} --config
          "${VITIS_CONFIGURATION_FILE}"
          "${CMAKE_BINARY_DIR}/${VITIS_KERNEL_AUX_NAME}.xo" -o
          "${CMAKE_BINARY_DIR}/${BINARY_NAME}")

      if(NOT ${VITIS_KERNEL_AUX_TYPE} STREQUAL "hw")
        # get a temporary symlink pointing the binary name to allow/permit the
        # v++ packaging process NOTE: v++ is sensitive to the artifact names.
        # See ERROR: [v++ 60-2262] Unsupported input file type specified for
        # --package option.
        message("ln -s ${BINARY_NAME} ${VITIS_KERNEL_AUX_NAME}.xclbin")
        execute_process(COMMAND ln -s ${BINARY_NAME}
                                ${VITIS_KERNEL_AUX_NAME}.xclbin)
      endif()

      # package only makes sense if linked already
      if(${VITIS_KERNEL_AUX_PACKAGE})
        # message("packaging")  # debug
        execute_process(
          COMMAND
            ${VPP_PATH} -p -t ${VITIS_KERNEL_AUX_TYPE} --config
            "${VITIS_CONFIGURATION_FILE}"
            "${CMAKE_BINARY_DIR}/${VITIS_KERNEL_AUX_NAME}.xclbin"
            --package.out_dir package --package.no_image)
      endif() # package

      #
      # dfx-mgr artifacts
      #
      # NOTE: dfx-mgr implementation in 2020.2.2 requires besides the xilinx
      # binary kernel (.xclbin), to provide also two additional files: -
      # .bit.bin raw bitstream - .dtbo device tree blob overlay
      #
      # NOTE 2: dfx-mgr 2021.2 introduces changes in the requirements of the
      # files. - .bit.bin bootgen-generated file - .dtbo device tree blog
      # overlay
      #
      ...
      if(EXISTS ${CMAKE_BINARY_DIR}/${BINARY_NAME})
        ...
        install(
          FILES "${CMAKE_BINARY_DIR}/${BINARY_NAME}"
                "${CMAKE_BINARY_DIR}/${VITIS_KERNEL_AUX_NAME}.dtbo"
        ...
        if(${VITIS_KERNEL_AUX_PACKAGE})
          ...
          # if hw, symlink to BOOT.BIN NOTE: removes and creates a new symlink
          # for each kernel, only the last symlinked kernel will remain in
          # firmware/ dir.
          #
      ...
      if(NOT ${VITIS_KERNEL_AUX_TYPE} STREQUAL "hw")
        # # Check before unlinking, see below execute_process( COMMAND unlink
        # ${VITIS_KERNEL_AUX_NAME}.xclbin )
